...
export class BoardController {
    ...
    async nowayList(@Query() paginationDto : PaginationDto) {
        ...
        const result : BoardResDto[] = await this.boardService.getListAll(paginationDto)
        return {
            data : result
        }
    }
    ...
    async boardInput(@Param() getByCashbookIdDto : GetByCashbookIdDto, @Body() postBoardDto : PostBoardDto, @Req()req : any) {
    try {
        let boardTypes : number;
        let message : string;
        let { user } = req
        const cashbook  = await this.cashbookService.getcashbookAndDetail(getByCashbookIdDto)
    ...
    @Get('detail/:boardId')
    async boardDetail(@Param() getByBoardIdDto : GetByBoardIdDto, @Req() req : Request) {
    ...
    async boardDelete(@Param() GetByBoardIdDto : GetByBoardIdDto) {
        const result = await this.boardService.deleteByboardId(GetByBoardIdDto.boardId)
        if(result) {
            return `삭제에 성공하였습니다`
        } else {
