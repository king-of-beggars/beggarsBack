...
export class CommentController {
    ...
    async postComment(@Param() getByBoardIdDto : GetByBoardIdDto, @Body() commentText : string, @Req() req : any) {
        let { user } = req
    ...
    async deleteComment(@Param() getByCommentIdDto : GetByCommentIdDto, @Req() req : any) {
        let { user } = req
        const result : DeleteResult = await this.commentService.deleteComment(getByCommentIdDto,user.userId)
        if(!result) {
