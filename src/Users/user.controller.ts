...
import { NaverAuthenticationGuard } from './passport/naver/naver.guard';
...
export class UserController {
  ...
  async kakaoLogin(@Query() code, @Req() req: any, @Res() res: Response) {
    ...
    if (!user.userId) {
      ...
      res.setHeader(
        'Set-Cookie',
        `userName=${user}; Path=/; host=https://beggars-front.vercel.app; sameSite=none; secure=true; httpOnly=false;`,
      );
      return res.redirect(
        `https://beggars-front.vercel.app?loginSuccess=false`,
      );
    }
  ...
  @Get('login/naver')
  @UseGuards(NaverAuthenticationGuard)
  @HttpCode(200)
  async naverLogin(@Query() code, @Req() req: any, @Res() res: Response) {
    const { user } = req;
    console.log(user);
    if (!user) {
      throw new Error('잘못된 접근입니다');
