...
export class CashbookContoller {
  ...
  async mainPage(@Req() req: any) {
    ...
    //1. 몇 일 째 되는 날
    const dateValue: number = await this.userService.userSignupDate(
      user.userId,
    );
    console.log(dateValue);
    //2. 2주 데이터, 남은 날은 null 처리
    const twoweek = await this.cashbookService.getCashbookDuringDate(
      nowdate,
      user.userId,
    );

    //3. 당일 유저 별, 섹션 별 총합목표, 총합소비
    const totalValue = await this.cashbookService.getCashbookByDate(
      nowdate,
      user.userId,
    );
  ...
  @Post(':cashDetailId')
  async postDetail(
    @Param() params: CashbookEntity,
    @Body() body: PostDetailDto,
  ) {
